...
if (view === 'dashboard') {
    ...
    // 1. Variablen & Gewinner-Logik definieren
    const unclaimedWin = myDuels.find(d => {
      ...
      // Mehr Punkte oder gleiche Punkte + schneller
    ...
    // Diese Variable brauchst du gleich im Home-Screen, daher steht sie hier oben
    const myOpenDuels = myDuels.filter(d => d.creator === user.name && d.status === 'open');

    // ---------------------------------------------------------
    // NEU: VIEW - BADGES / HALL OF FAME
    // ---------------------------------------------------------
   ...
   // HIER GEHT DEIN CODE WEITER (z.B. mit if dashboardView === 'home')
...
if (dashboardView === 'home') {
  ...
  if (dashboardView === 'history') {
    ...
    if (dashboardView === 'badges') {
      ...
      // 1. STATISTIK BERECHNEN (Live aus der Historie)
      let stats = {
      ...
      myDuels.forEach(d => {
         ...
         // Nur fertige Spiele zählen für Wins/Sats
         if (d.status === 'finished') {
             // Win Check
             const iWon = myScore > opScore || (myScore === opScore && myTime < opTime);
      ...
      const BADGES = [
        ...
        { id: 'p1', name: 'Neuling', desc: '5 Spiele gespielt', icon: User, color: 'text-blue-400', achieved: stats.played >= 5 },
        { id: 'p2', name: 'Stammgast', desc: '25 Spiele gespielt', icon: Users, color: 'text-blue-500', achieved: stats.played >= 25 },
        { id: 'p3', name: 'Inventar', desc: '50 Spiele gespielt', icon: Globe, color: 'text-blue-600', achieved: stats.played >= 50 },
