...
🌟 Features
⚡ Lightning Native: Seamless deposits and withdrawals using LNbits integration.
...
Bash
git clone https://github.com/louisthecat86/SatoshiDuell.git
cd SatoshiDuell
npm install
...
VITE_LNBITS_URL=https://legend.lnbits.com (or your instance)
VITE_INVOICE_KEY=your_lnbits_invoice_key_for_deposits

# Backend (Vercel Serverless Functions)
# These are NOT prefixed with VITE_ and remain hidden from the browser
LNBITS_ADMIN_KEY=your_lnbits_admin_key_for_withdrawals
DONATION_LN_ADDRESS=mw860602@blink.sv
...
create table public.duels (
  ...
  status text not null default 'open'::text, -- open, finished, refunded
  creator_score numeric null,
  creator_time numeric null,
  challenger_score numeric null,
  challenger_time numeric null,
  questions jsonb null,
  claimed boolean null default false,
  target_player text null, -- for private challenges
...
Bash
npm run dev
Note: The API functions (Refund, Donate, Load Questions) run via Vercel Serverless. To test them locally, you need vercel dev instead of npm run dev, or you deploy to Vercel.

🎮 How to Play
Login: Choose a Gamertag (PIN secured) or use your Nostr Extension.
...
⚠️ Disclaimer
SatoshiDuell involves real money (Bitcoin/Satoshis). This software is provided "as is", without warranty of any kind. The developer is not responsible for any lost funds due to software bugs, network errors, or user mistakes. Use at your own risk and only play with what you can afford to lose.

❤️ Support the Dev
If you enjoy this project and want to support the development:
